// ============================================================================
// userController.js - Quản lý NGƯỜI DÙNG (users)
// ============================================================================
// FLOW: User đăng ký → POST /api/users → User.create()
//       Admin xem users → GET /api/users → User.findAll()
//       User xem profile + progress → GET /api/users/:id/progress →
//       → Order.findByUserId() → Course.findById() → Progress.getCompletedLessons()
//
// 8 functions: CRUD (create, getAll, getById, update, delete) + getUserProgressDetails,
// updateUserRole, verifyStudent
...
exports.createUser = async (req, res) => {
    try {
        // Sử dụng User Model
        const newUser = await User.create(req.body); // GỌI MODEL ⭐
...
exports.getUsers = async (req, res) => {
    try {
        ...
        const users = await User.findAll(filters); // GỌI MODEL ⭐
...
exports.getUserById = async (req, res) => {
    try {
        // Sử dụng User Model
        const user = await User.findById(req.params.id); // GỌI MODEL ⭐
...
exports.updateUser = async (req, res) => {
    try {
        // Sử dụng User Model
        const updatedUser = await User.update(req.params.id, req.body); // GỌI MODEL ⭐
...
exports.deleteUser = async (req, res) => {
    try {
        ...
        await User.delete(req.params.id); // GỌI MODEL ⭐
...
exports.getUserProgressDetails = async (req, res) => {
    try {
        ...
        const userOrders = await Order.findByUserId(userId); // GỌI MODEL ⭐
        ...
        for (const order of userOrders) {
            try {
                ...
                const lessons = await Lesson.findByCourseId(courseId); // GỌI MODEL ⭐
                const totalLessons = lessons.length;
                console.log(`Course ${courseId} has ${totalLessons} total lessons`);

                // Get completed lessons using Progress model
                const completedLessonsData = await Progress.getCompletedLessons(userId, courseId); // GỌI MODEL ⭐
...
exports.updateUserRole = async (req, res) => {
    try {
        ...
        const user = await User.findById(userId); // GỌI MODEL ⭐
        ...
        const updatedUser = await User.update(userId, { role: role }); // GỌI MODEL ⭐
...
exports.verifyStudent = async (req, res) => {
    try {
        ...
        const updatedUser = await User.verifyAsStudent(user_id, email); // GỌI MODEL ⭐
...
// ============================================================================
// TÓM TẮT: CRUD users + getUserProgressDetails (tính % học xong từ orders),
// updateUserRole (admin phân quyền), verifyStudent (xác minh email .edu)
// KEY: new Set() prevent duplicates, Math.max/min/round for calculations,
// .sort() with custom logic, req.user.role for auth check
// ============================================================================
